
import { useMemo } from 'react';
import { Bar, BarChart, XAxis, YAxis, LabelList, Cell } from 'recharts';
import {
    type ChartConfig,
    ChartContainer,
    ChartTooltip,
    ChartTooltipContent,
} from '@/components/ui/chart';

interface VoteChartProps {
    options: { id: number; optionText: string }[];
    counts: { voteOptionId: number; count: number }[];
    loading?: boolean;
}

export default function VoteChart({ options, counts }: VoteChartProps) {
    console.log('VoteChart data:', options, counts);

    const data = useMemo(() => {
        const map = new Map(counts.map(c => [c.voteOptionId, c.count]));

        const rows = options.map((o, index) => ({
            id: o.id,
            name: o.optionText,
            count: map.get(o.id) ?? 0,
            // CSS 변수로 다시 시도하려면 아래 주석 해제
            fill: `hsl(var(--chart-${(index % 5) + 1}))`,
        }));

        console.log('Chart rows with fill:', rows); // ✅ fill 값 확인용

        const total = rows.reduce((s, r) => s + r.count, 0);
        return {
            total,
            rows: rows.map(r => ({
                ...r,
                pct: total ? Math.round((r.count / total) * 1000) / 10 : 0
            }))
        };
    }, [options, counts]);

    if (data.rows.length === 0) return null;

    // ✅ ChartConfig 단순화
    const chartConfig: ChartConfig = {
        count: {
            label: "투표수",
        },
    };

    return (
        <div className="w-full p-3 rounded-xl border bg-background mb-3">
            <div className="flex items-center justify-between mb-2">
                <span className="text-sm text-muted-foreground">현재 투표 현황</span>
                <span className="text-sm text-muted-foreground">{data.total}명 투표</span>
            </div>
            {data.total === 0 ? (
                <div className="text-sm text-muted-foreground py-6 text-center">
                    아직 투표가 없습니다.
                </div>
            ) : (
                <ChartContainer config={chartConfig} className="h-36 w-full">
                    <BarChart
                        data={data.rows}
                        layout="vertical"
                        margin={{ top: 4, right: 30, bottom: 4, left: 8 }}
                    >
                        <XAxis type="number" hide domain={[0, 'dataMax']} />
                        <YAxis
                            type="category"
                            dataKey="name"
                            width={80}
                            tick={{ fontSize: 12 }}
                            tickFormatter={(v) => (v.length > 6 ? v.slice(0, 6) + '…' : v)}
                        />
                        <ChartTooltip
                            content={<ChartTooltipContent
                                formatter={(value, _name, props) => {
                                    const pct = props?.payload?.pct ?? 0;
                                    return [`${value}표 (${pct}%)`, ''];
                                }}
                                labelFormatter={(label) => label}
                            />}
                        />
                        <Bar
                            dataKey="count"
                            radius={[6, 6, 6, 6]}
                            fill="#8884d8" // ✅ 기본 색상 추가
                        >
                            {/* ✅ Cell로 개별 색상 적용 */}
                            {data.rows.map((entry, index) => (
                                <Cell
                                    key={`cell-${index}`}
                                    fill={entry.fill}
                                />
                            ))}
                            <LabelList
                                dataKey="pct"
                                position="right"
                                className="text-[10px] fill-foreground"
                            />
                        </Bar>
                    </BarChart>
                </ChartContainer>
            )}
        </div>
    );
}